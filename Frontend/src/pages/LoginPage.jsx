    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 200 && response.data.message === "Connexion réussie!") {
                ...
                // Si une destination est dans l'URL, on redirige vers la réservation avec l'ID utilisateur
                if (destinationId) {
                    navigate(`/reservation?destination=${destinationId}&user=${userId}`);
                } else {
                    navigate('/'); // Sinon, redirige vers la page d'accueil ou un autre endroit
                }
            }
        } catch (error) {
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <Box sx={{ marginBottom: 2 }}>
                            <TextField
                                ...
                                error={!!error}
                                helperText={error && "Le mot de passe est requise"}
                                required
                                InputProps={{
                                    endAdornment: (
                                        <IconButton
